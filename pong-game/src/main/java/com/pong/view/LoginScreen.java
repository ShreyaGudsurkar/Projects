...
public class LoginScreen extends JPanel {
    ...
    private void loginAction(boolean isPlayer1) {
        ...
        try {
            User user = userManager.authenticateUser(username, password); // Authenticate user
            JOptionPane.showMessageDialog(frame, username + " logged in successfully!");

            // Store the userId for the logged-in player
            if (isPlayer1) {
            ...
            if (isPlayer1LoggedIn && isPlayer2LoggedIn) {
                ...
                GameView gameView = new GameView(playerOne, playerTwo); // Pass both userIds to GameView
                GameController controller = new GameController(gameView, frame, playerOne, playerTwo);
                frame.add(gameView);
