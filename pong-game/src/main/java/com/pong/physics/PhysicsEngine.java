...
public class PhysicsEngine {
    // Physics constants
    private final int initialSpeed;           // Initial speed of the ball
    private final int maxDeformationFrames;   // Frames to display ball deformation
    private final double paddleInfluence;     // Influence of paddle movement on ball

    // Reference to the game model to update positions and speeds
     ...
     */
    public PhysicsEngine(GameModel model, int initialSpeed, int maxDeformationFrames) {
        this.model = model;
        this.initialSpeed = initialSpeed;
        this.maxDeformationFrames = maxDeformationFrames;
        this.paddleInfluence = 0.35; // Influence of paddle movement on ball's vertical speed
    ...
    private void handlePaddleCollision(Rectangle ballRect, Rectangle paddleRect, int paddleSpeed, boolean isLeftPaddle) {
        if (ballRect.intersects(paddleRect)) {
            // Collision detected between ball and paddle
            playPaddleHitSound();
    ...
    private void playPaddleHitSound() {
        if (paddleHitClip != null) {
            paddleHitClip.setFramePosition(0); // Rewind the clip to the beginning
            paddleHitClip.start(); // Play the sound
        }
    }
    ...
    public void resetBall() {
        ...
        // Randomize initial ball direction (left/right and up/down)
        model.ballXSpeed = initialSpeed * (Math.random() > 0.5 ? 1 : -1);
        model.ballYSpeed = initialSpeed * (Math.random() > 0.5 ? 1 : -1);

        // Reset deformation frames (no deformation at the start)
